...
# CJH had to update the imports for tk - 2022 0211
from tkinter import *
from math import *
import os
...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        self.ShapeVar.set(0)
        Radiobutton(self.EntryFrame, text='Circle', variable=self.ShapeVar, value=0, command=self.DoIt, indicatoron=0,
                    width=5, ).grid(row=self.rownumber + 1, column=0, sticky=W)
        Radiobutton(self.EntryFrame, text=' Rect ', variable=self.ShapeVar, value=2, command=self.DoIt, indicatoron=0,
                    width=5, ).grid(row=self.rownumber + 2, column=0, sticky=W)

        self.stPS = Label(self.EntryFrame, text='Path', width=7)
        self.stPS.grid(row=self.rownumber, column=0, sticky=E)
        self.PathVar = IntVar()
        self.PathVar.set(0)
        Radiobutton(self.EntryFrame, text='Paralell', variable=self.PathVar, value=0, command=self.DoIt, indicatoron=0,
                    width=7, ).grid(row=self.rownumber + 1, column=0, sticky=E)
        Radiobutton(self.EntryFrame, text=' Spiral ', variable=self.PathVar, value=1, command=self.DoIt, indicatoron=0,
                    width=7, ).grid(row=self.rownumber + 2, column=0, sticky=E)
        ...
        Radiobutton(self.EntryFrame, text='Zero', variable=self.MovmentVar, value=0, command=self.DoIt, indicatoron=0,
                    width=7, ).grid(row=self.rownumber + 1, column=1, sticky=W, padx=10)
        Radiobutton(self.EntryFrame, text='G41/G42', variable=self.MovmentVar, value=1, command=self.DoIt,
                    indicatoron=0, width=7, ).grid(row=self.rownumber + 2, column=1, sticky=W, padx=10)
        ...
        self.DirVar.set('CCW')
        Radiobutton(self.EntryFrame, text='CCW', variable=self.DirVar, value='CCW', command=self.DoIt, indicatoron=0,
                    width=7, ).grid(row=self.rownumber + 1, column=1, sticky=N)
        Radiobutton(self.EntryFrame, text='CW', variable=self.DirVar, value='CW', command=self.DoIt, indicatoron=0,
                    width=7, ).grid(row=self.rownumber + 2, column=1, sticky=N)

        self.st13 = Label(self.EntryFrame, text='Units', width=5)
        self.st13.grid(row=self.rownumber, column=1, sticky=E)
        self.UnitVar = IntVar()
        self.UnitVar.set(0)  # 0 is inches
        Radiobutton(self.EntryFrame, text='Inch', variable=self.UnitVar, value=0, command=self.Change_Units,
                    indicatoron=0, width=5, ).grid(row=self.rownumber + 1, column=1, sticky=E)
        Radiobutton(self.EntryFrame, text=' MM ', variable=self.UnitVar, value=1, command=self.Change_Units,
                    indicatoron=0, width=5, ).grid(row=self.rownumber + 2, column=1, sticky=E)
        ...
        self.ToolDiameterVar.set('0.375')
        ...
        self.ToolNumberVar.set('2')
        self.ToolNumber = Spinbox(self.EntryFrame, textvariable=self.ToolNumberVar, from_=0, to=99, width=4,
        ...
        self.ToolOffsetNumberVar.set('2')
        ...
        self.Finish = IntVar()
        self.Finish.set(1)
        Checkbutton(self.EntryFrame, text='Finish Contour', variable=self.Finish, onvalue=1, offvalue=0,
                    command=self.DoIt).grid(row=self.rownumber, column=1, sticky=E)
    ...
    def ChangeTool(self):
        global toolChange
        print
        'Inside ChangeTool T:' + str(self.ToolNumberVar.get())
    ...
    def DoIt(self):
        ...
        float(self.XPocketCenterVar.get()), float(self.YPocketCenterVar.get())))  # G0 CenterZsafe
        # get the inputs
        ...
        if self.ShapeVar.get() == 0:  # circel
            ...
            if self.PathVar.get() == 0:  # cricel parallell path
                ...
                while tempz < 0:
                    ...
                    for cir in range(1, numberofcircels):
                    ...
                    if self.Finish.get() == 1:
                        ...
                        self.CanvasDrawings.append(
                            self.PreviewCanvas.create_line(150, 150, 150 + corner, 150, fill='red'))
                        self.CanvasDrawings.append(
                            self.PreviewCanvas.create_oval(150 - corner, 150 - corner, 150 + corner, 150 + corner,
                                                           outline='Red'))
                        # Gcode finish
                    ...
                    float(self.XPocketCenterVar.get()), float(self.YPocketCenterVar.get())))  # Center
                self.gcode.append('G0 Z%.4f ' % (float(self.SafeZVar.get())))  # G0 Zsafe
            else:  # cirel_spiral
                ...
                self.CanvasDrawings.append(
                    self.PreviewCanvas.create_line(150, 150, 150 + (Spacing / Scale), 150, fill='red'))
                for arc_curve in range(1, numberofcircels + 1):
                    if arc_curve % 2 == 0:  # geradezahl
                        if arc_curve == 2:
                            # canvas_new_Y =150-((arc_curve*(versatz/2.0))/scale)
                            self.CanvasDrawings.append(
                                self.PreviewCanvas.create_arc(canvas_new_Y, canvas_new_Y, 300 - canvas_new_Y + vsh,
                                                              300 - canvas_new_Y, start=180, extent=180, outline='red',
                                                              style=ARC))
                        else:
                            canvas_new_Y = 150 - ((arc_curve * (Spacing / 2.0)) / Scale)
                            self.CanvasDrawings.append(self.PreviewCanvas.create_arc(canvas_new_Y + vsh, canvas_new_Y,
                                                                                     300 - canvas_new_Y + vsh,
                                                                                     300 - canvas_new_Y, start=180,
                                                                                     extent=180, outline='red',
                                                                                     style=ARC))
                    else:
                        if arc_curve == 1:
                            canvas_new_Y = 150 - ((arc_curve * Spacing) / Scale)
                            self.CanvasDrawings.append(
                                self.PreviewCanvas.create_arc(canvas_new_Y, canvas_new_Y, 300 - canvas_new_Y,
                                                              300 - canvas_new_Y, start=0, extent=180, outline='red',
                                                              style=ARC))
                        else:
                            canvas_new_Y = 150 - ((arc_curve * (Spacing / 2.0)) / Scale)
                            self.CanvasDrawings.append(
                    ...
                    self.CanvasDrawings.append(
                        self.PreviewCanvas.create_line(150, 150, 150 + Radius - (vsh / 2), 150, fill='red'))
                self.CanvasDrawings.append(
                    self.PreviewCanvas.create_text((150, 280), text="Grafic may differ to G_code on some Enterys"))
                if self.Finish.get() == 1:
                    Radius = (PocketRadius - ToolDiameterRad) / Scale
                    self.CanvasDrawings.append(self.PreviewCanvas.create_oval(
                        150 - Radius + (vsh / 2),
                ...
                while tempz < 0:
                    ...
                    self.gcode.append(
                        'G3 X%.3f Y%.3f I-%.3f ' % (centerx - PocketRadius + ToolDiameterRad, centery, newI))
                    self.gcode.append('G3 I%.3f ' % (PocketRadius - ToolDiameterRad))
                    # go to Zsafe before moving to center
                    if (tempz >= 0):
                        self.gcode.append('G0 Z%.4f ' % (float(self.SafeZVar.get())))  # G0 Zsafe
            ...
            if self.PathVar.get() == 0:  # Parallel
                ...
                while tempz < 0:
                    ...
                    while while_value < rec_cycels:
                        ...
                        self.CanvasDrawings.append(self.PreviewCanvas.create_rectangle(
                            150 - canvas_new_X, 150 - canvas_new_Y, 150 + canvas_new_X, 150 + canvas_new_Y,
                            outline='red'))
                        self.gcode.append(
                            'G1 G90 X%.3f Y%.3f F%.2f ' % (centerx + first_length_x + (while_value * Spacing)
                                                           , centery + first_length_y + (while_value * Spacing),
                                                           speedXY))
                        self.CanvasDrawings.append(
                            self.PreviewCanvas.create_line(canvasoldX, canvasoldY, 150 + canvas_new_X,
                                                           150 - canvas_new_Y, fill='red'))
                    ...
                    if self.Finish.get() == 1:
                        self.gcode.append('G1 G90 X%.3f Y%.3f ' % (
                        (centerx + (pocket_length_x / 2.0) - ToolDiameterRad),
                        (centery + (pocket_length_y / 2.0) - ToolDiameterRad)))
                        self.CanvasDrawings.append(self.PreviewCanvas.create_line(canvasoldX, canvasoldY,
                                                                                  150 + ((
                                                                                                     pocket_length_x - ToolDiameter) / 2.0 / Scale),
                                                                                  150 - ((
                                                                                                     pocket_length_y - ToolDiameter) / 2.0 / Scale),
                                                                                  fill='red'))
            ...
            # ----------
                ...
                while tempz < 0:
                    ...
                    while cycle < rec_cycels:
                        ...
                        self.CanvasDrawings.append(
                            self.PreviewCanvas.create_line(canvasoldX, canvasoldY, canvas_new_X, canvasoldY,
                                                           fill='red'))
                        canvasoldX = canvas_new_X
                        self.gcode.append('G1 Y%.3f ' % (tempy))
                        canvas_new_Y = canvasoldY - (tempy / Scale)
                        self.CanvasDrawings.append(
                            self.PreviewCanvas.create_line(canvasoldX, canvasoldY, canvasoldX, canvas_new_Y,
                                                           fill='red'))
                    ...
                    self.CanvasDrawings.append(
                        self.PreviewCanvas.create_line(canvasoldX, canvasoldY, canvas_new_X, canvasoldY, fill='red'))
                    canvasoldX = canvas_new_X
                    if self.Finish.get() == 1:
                        self.gcode.append('G1 G90 X%.3f Y%.3f ' % (
                        (centerx + (pocket_length_x / 2.0) - ToolDiameterRad),
        ...
        if self.MovmentVar.get() == 1:
            messagebox.showinfo("Pocket V1", " G41/42 Not implemented ")
            self.MovmentVar.set(0)
            # Tool on canvas
        self.CanvasDrawings.append(self.PreviewCanvas.create_oval(
