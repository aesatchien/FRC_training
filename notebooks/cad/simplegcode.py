...
settings_dict = {
    'shapeoko':{'DRILL_DEPTH':-0.12, 'RETRACT_HEIGHT':0.5, 'ENGAGE_HEIGHT':0.1,
                'LINEAR_FEED':100, 'DRILL_FEED':5, 'RETRACT_FEED':30,},
    'benchmill':{'DRILL_DEPTH':-0.2, 'RETRACT_HEIGHT':0.5, 'ENGAGE_HEIGHT':0.1,
                'LINEAR_FEED':30, 'DRILL_FEED':3, 'RETRACT_FEED':20, 'TOOL':4, 'PECK_TOOL':5,
                 'PECK_TOP':0.1, 'PECK_BOTTOM':-0.25, 'PECK_STEP':0.05},
}
...
def generate_gcode(point_list, machine='shapeoko', overrides=None, peck=False, save_file=False, file_description='test'):
    ...
    if machine == 'shapeoko':
        intro_code = f'; ## CENTER DRILL CODE AUTOGENERATED FROM POINT LIST FOR SHAPEOKO- CJH {datetime.today().strftime("%Y %m%d %H:%M:%S")}'
        ...
        intro_code = f'; ## CENTER DRILL CODE AUTOGENERATED FROM POINT LIST FOR BENCHMILL- CJH {datetime.today().strftime("%Y %m%d %H:%M:%S")}'
    ...
    if peck:  # canned pecking cycle
        ...
        message = message + f'N0034 G83 G98 R{settings_dict[machine]["PECK_TOP"]} Z{settings_dict[machine]["PECK_BOTTOM"]} Q{settings_dict[machine]["PECK_STEP"]} F{settings_dict[machine]["DRILL_FEED"]}\n'
        for ix, point in enumerate(sorted_list[1:]):
            # move to x,y, drill, retract
            # message = message + f'; ##POINT #{ix+1}:\n'
...
def plot_points(point_list, annotate=True, show_order=True):
